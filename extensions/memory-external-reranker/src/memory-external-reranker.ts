 ...
 * llama.cpp /v1/rerank, etc.). The provider ID is taken from the plugin config and
 * must match an entry in `models.providers` in the OpenClaw config. Iterates model
 * candidates in order and returns on the first success; aggregates errors and throws
...
export class ExternalMmrReranker implements MemoryRerankerPlugin {
  ...
  async rerank(params: RerankParams): Promise<RerankResult> {
    ...
    const allowPrivateNetwork = providerEntry.request?.allowPrivateNetwork === true;
